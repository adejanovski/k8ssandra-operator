...
func testMedusaTasks(t *testing.T, ctx context.Context, f *framework.Framework, namespace string) {
	...
	backup1Created := createAndVerifyBackup(dc1Key, dc1, f, ctx, require, t, namespace, backup1)
	require.True(backup1Created, "failed to create backup1")
	backup2Created := createAndVerifyBackup(dc1Key, dc1, f, ctx, require, t, namespace, backup2)
	require.True(backup2Created, "failed to create backup2")
	backup3Created := createAndVerifyBackup(dc1Key, dc1, f, ctx, require, t, namespace, backup3)
