...
func TestImages(t *testing.T) {
	...
	t.Run("nil images", func(t *testing.T) {
		...
		deployment := NewDeployment(reaper, newTestDatacenter(), nil, nil)
		assert.Equal(t, "docker.io/thelastpickle/cassandra-reaper:config-heap-size", deployment.Spec.Template.Spec.InitContainers[0].Image)
		assert.Equal(t, "docker.io/thelastpickle/cassandra-reaper:config-heap-size", deployment.Spec.Template.Spec.Containers[0].Image)
